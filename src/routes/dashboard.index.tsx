...
function DashboardOverview() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const [{ count: contacts }, { count: dueToday }, { count: openTouches }, { data: prof }] =
        await Promise.all([
          supabase
  ...
  return (
    <div className="space-y-10 max-w-6xl">
      ...
      <div className="grid sm:grid-cols-3 gap-4">
        <StatCard
          icon={<Users />}
          label="Contacts You've Added"
          value={stats.contacts}
          link="/dashboard/evangelism"
        />
        <StatCard
          icon={<Bell />}
          label="Follow-ups Due"
          value={stats.dueToday}
          link="/dashboard/evangelism/follow-ups"
          highlight={stats.dueToday > 0}
        />
        <StatCard
          icon={<Calendar />}
          label="Open Follow-ups"
          value={stats.openTouches}
          link="/dashboard/evangelism/follow-ups"
        />
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <Link
          ...
          className="block bg-night text-night-foreground p-8 hover:bg-night/90 transition-colors"
        >
          ...
          <p className="text-night-foreground/70 text-sm">
            Track everyone we meet, and never miss a follow-up.
          </p>
        </Link>
        <Link
          to="/dashboard/evangelism/follow-ups"
          className="block bg-secondary text-foreground p-8 hover:bg-secondary/80 transition-colors"
        >
          <div className="eyebrow text-accent mb-4">— Today</div>
          <div className="font-display text-3xl mb-2">Your follow-ups</div>
          <p className="text-muted-foreground text-sm">
            The follow-ups you set for yourself, with the note behind each one.
          </p>
        </Link>
      </div>
    </div>
...
function StatCard({
  ...
  return (
    <Link
      to={link as any}
      className={`block border p-6 transition-colors ${highlight ? "border-accent bg-accent/5" : "border-border bg-card hover:border-foreground/20"}`}
    >
