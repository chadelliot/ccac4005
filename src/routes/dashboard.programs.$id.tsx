...
function ProgramDetail() {
  ...
  const toggleComplete = async (day: Day) => {
    ...
    if (progress.has(day.id)) {
      ...
      await supabase
        .from("reading_program_progress")
        .insert({ user_id: user.id, program_id: id, day_id: day.id });
    }
    load();
  };
  ...
  const enroll = async () => {
    ...
    const { error } = await supabase
      .from("program_enrollments")
      .insert({ program_id: id, user_id: user.id });
    if (error) return toast.error(error.message);
  ...
  const downloadCert = async () => {
    ...
    downloadCertificatePdf({
      ...
      memberName,
      completionDate: new Date().toLocaleDateString(undefined, {
        year: "numeric",
        month: "long",
        day: "numeric",
      }),
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-3 mt-3 flex-wrap">
            <Badge variant={program.status === "published" ? "secondary" : "outline"}>
              {program.status}
            </Badge>
            {program.estimated_duration && (
              <Badge variant="outline">{program.estimated_duration}</Badge>
            )}
            {program.includes_quiz && <Badge variant="outline">Quiz</Badge>}
            {program.includes_certificate && (
        ...
        <div className="border border-border bg-card p-4 flex items-center justify-between gap-3 flex-wrap">
          {enrollment ? (
            <>
              <div className="flex-1 min-w-[200px]">
                ...
                <div className="text-xs text-muted-foreground mt-1">
                  {completedCount} of {days.length} days • {completion}%
                </div>
              </div>
              {certEarned && (
                <Button
                  onClick={downloadCert}
                  className="bg-night text-night-foreground rounded-none eyebrow gap-2"
                >
            ...
            <>
              ...
              <Button
                onClick={enroll}
                className="bg-night text-night-foreground rounded-none eyebrow"
              >
                Enroll
              </Button>
            </>
      ...
      <Tabs defaultValue="content">
        <TabsList>
          <TabsTrigger value="content">
            <BookOpen className="h-3 w-3 mr-1" /> Content
          </TabsTrigger>
                    ...
                    <DialogContent className="max-w-lg">
                      <DialogHeader>
                        <DialogTitle className="font-display text-2xl">
                          Add Day {(days[days.length - 1]?.day_number ?? 0) + 1}
                        </DialogTitle>
                      </DialogHeader>
                      <form onSubmit={handleAddDay} className="space-y-4">
                        ...
                        <DialogFooter>
                          <Button
                            type="submit"
                            disabled={busy}
                            className="w-full bg-night text-night-foreground rounded-none py-6 eyebrow"
                          >
                            {busy ? "Saving..." : "Add Day"}
                          </Button>
                        </DialogFooter>
                    ...
                    >
                      ...
                      {isOpen && (
                        <div className="border-t border-border p-4 bg-background space-y-4">
                          {day.summary && (
                            <p className="text-sm italic text-muted-foreground">{day.summary}</p>
                          )}
                          {day.passages?.map((p, i) => (
                            <PassageView key={i} passage={p} />
                          ))}
                          {day.reflection_question && (
                            <div className="border-l-2 border-accent pl-3 text-sm italic">
                              {day.reflection_question}
                            </div>
                          )}
                          {day.notes && (
                            <div className="text-sm whitespace-pre-wrap">{day.notes}</div>
                          )}
                        </div>
                      )}
                    </div>
    ...
    <div className="border border-border bg-card p-5 space-y-3">
      ...
      <div className="grid sm:grid-cols-2 gap-3">
        ...
        <div>
          ...
          <Input
            value={c.church_name}
            onChange={(e) => setC({ ...c, church_name: e.target.value })}
          />
        </div>
        <div>
          <Label>Member name on cert (optional default)</Label>
          <Input
            value={c.member_name}
            onChange={(e) => setC({ ...c, member_name: e.target.value })}
          />
        </div>
        <div>
          <Label>Signature name</Label>
          <Input
            value={c.signature_name}
            onChange={(e) => setC({ ...c, signature_name: e.target.value })}
          />
        </div>
        <div>
          <Label>Signature title</Label>
          <Input
            value={c.signature_title}
            onChange={(e) => setC({ ...c, signature_title: e.target.value })}
          />
        </div>
      </div>
      <div className="flex gap-2">
        <Button onClick={save} className="bg-night text-night-foreground rounded-none eyebrow">
          Save
        </Button>
  ...
  if (loading)
    return (
      <div className="text-sm text-muted-foreground">
        <Loader2 className="h-4 w-4 inline animate-spin" /> Loading...
      </div>
    );
        ...
        {enrollments.length === 0 ? (
          ...
          <div className="space-y-1">
            {enrollments.map((e) => (
              <div
                key={e.id}
                className="flex items-center justify-between text-sm border border-border p-2"
              >
                <span>{profiles[e.user_id]?.display_name ?? e.user_id.slice(0, 8)}</span>
                <span className="flex items-center gap-3 text-xs text-muted-foreground">
                  {totalDays} days
                  <span className="w-32">
                    <Progress value={Math.round(e.percent_complete)} className="h-1.5" />
                  </span>
